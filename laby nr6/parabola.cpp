...
//konstruktor kopiujacy obiekt "p" klasy point
point::point(point &p)
        : x(p.x), y(p.y), counter(p.counter) {}
...
void point::operator== (const point &p) {
    if(x == p.x && y == p.y){
        std::cout << "To sa te same puntky." << std::endl;
    }
    else{
        std::cout << "To nie sa te same puntkty." << std::endl;
    }
}
...
//konstruktor kopiujacy obiekt "para" klasy parabola
parabola::parabola(parabola &para)
        : p1(para.p1), p2(para.p2), p3(para.p3), licz_parabol(para.licz_parabol) {}
...
//definicja przeciazenia operatora ==, czyli teraz mozemy sprawdzic czy parabole sa takie same np. para1 == para2 i poda nam odpowiedni wynik
void parabola::operator== (const parabola &para) {
    if(a == para.a && b == para.b && c == para.c){
        std::cout << "To sa te same parabole." << std::endl;
    }
    else{
        std::cout << "To nie sa te same parabole." << std::endl;
    }
}